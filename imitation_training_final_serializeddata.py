...
class imitation:
    ...
    def train(self):
        ...
        for epoch in range(EPOCHS):
            action_file = open('_out/imitation_training_data_old_optimized.pkl','rb')
            states_file = open('_out/imitation_training_images_old_optimized.pkl','rb')
            for i in range(0,TRAIN_SIZE,TRAINING_BATCH_SIZE):
                ...
                for j in range(len(y)):
                    ...
                    steer_accuracy += 1-(abs(round(y[j,0].item(), 2)+1 - round(yhat[j,0].item(), 2)+1)/(round(y[j,0].item(), 2)+1))
                    throttle_accuracy += 1-(abs(round(y[j,1].item(), 2)+1 - round(yhat[j,1].item(), 2)+1)/(round(y[j,1].item(), 2)+1))
                    brake_accuracy += 1-(abs(round(y[j,2].item(), 2)+1 - round(yhat[j,2].item(), 2)+1)/(round(y[j,1].item(), 2)+1))
            ...
            with torch.no_grad():
                for i in range(TRAIN_SIZE,TOTAL_LENGTH):
                    ...
                    for j in range(len(y_test)):
                        test_correct_steer += 1-(abs(round(y[j,0].item(), 2)+1 - round(yhat[j,0].item(), 2)+1)/(round(y[j,0].item(), 2)+1))
