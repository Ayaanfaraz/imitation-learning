...
class imitation:
    ...
    def train(self):
        ...
        with open('_out/data_new2_balanced.pkl','rb') as f:
        ...
        for i in shuffled_list:
            #print(images_list[i][0].size)
            images = np.asarray(images_list[i][0])
            print(type(images))
            print(images.shape)
            print(images.size)
            semantic_state = (torch.from_numpy(images).permute(2,0,1)/255)#.to(device)
            rgb_state = (torch.from_numpy(images).permute(2,0,1)/255)#.to(device)

            semantic_states.append(semantic_state) #Add the rgb/semantic segmented tuple
        ...
        for lrate in [0.0005]:
            ...
            for epoch in range(EPOCHS):
                ...
                for i in range(0,TRAIN_SIZE,TRAINING_BATCH_SIZE):
                    ...
                    x2    = torch.stack((rgb_states[i:i+int(TRAINING_BATCH_SIZE)]))  #states[i][1]#Uncertainty
...
######Validating the model
                ...
                with torch.no_grad():
                    for i in range(TRAIN_SIZE,len(shuffled_list)):
                        ...
                        x2_test    = torch.unsqueeze(rgb_states[i], 0)  #states[i][1]#Uncertainty
            ...
            tb.close()
            torch.save(self.model,f'imitation_models/model_final_lr_{lrate}_discrete.pt')
            torch.cuda.empty_cache()
