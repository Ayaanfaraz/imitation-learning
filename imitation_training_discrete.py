...
class imitation:
    def __init__(self):
        ...
        #print("Total layers: ",count)
        #self.loss = nn.MSELoss()
        self.optimizer = None #optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=0.0005)
    ...
    def train(self):
        ...
        data_filename ='_out/imitation_data_512.pkl' #'_out/imitation_training_data_55K.pkl' #'_out/imitation_data_512.pkl'
        images_filename ='_out/imitation_images_512.pkl' #'_out/imitation_training_images_55K.pkl' #'_out/imitation_images_512.pkl'
        ...
        for lrate in [0.0005]:
            ...
            for epoch in range(EPOCHS):
                ...
                for i in range(0,TRAIN_SIZE,TRAINING_BATCH_SIZE):
                    ...
                    rmse_steer_Y, rmse_steer_Yhat = [], []
                    rmse_throttle_Y, rmse_throttle_Yhat = [], []
                    rmse_brake_Y, rmse_brake_Yhat = [], []
                    ...
                    for j in range(len(y)):
                        ...
                        rmse_steer_Y.append(y[j,0].item())
                        rmse_throttle_Y.append(y[j,1].item())
                        rmse_brake_Y.append(y[j,2].item())

                        rmse_steer_Yhat.append(yhat[j,0].item())
                        rmse_throttle_Yhat.append(yhat[j,1].item())
                        rmse_brake_Yhat.append(yhat[j,2].item())
                        print("Tensor: ", torch.round(y[j,2], decimals=2)) 
                    del y
                    del yhat

                    rmse_steer_Y = torch.Tensor(rmse_steer_Y)
                    rmse_throttle_Y = torch.Tensor(rmse_throttle_Y)
                    rmse_brake_Y = torch.Tensor(rmse_brake_Y)

                    rmse_steer_Yhat = torch.Tensor(rmse_steer_Yhat)
                    rmse_throttle_Yhat = torch.Tensor(rmse_throttle_Yhat)
                    rmse_brake_Yhat = torch.Tensor(rmse_brake_Yhat)
                    ...
                    tb.add_scalar("Steer RMSE", math.sqrt(mean_squared_error(rmse_steer_Y, rmse_steer_Yhat)), epoch_itter)
                    tb.add_scalar("Throttle RMSE", math.sqrt(mean_squared_error(rmse_throttle_Y, rmse_throttle_Yhat)), epoch_itter)
                    tb.add_scalar("Brake RMSE", math.sqrt(mean_squared_error(rmse_brake_Y, rmse_brake_Yhat)), epoch_itter)
...
######Validating the model
                ...
                with torch.no_grad():
                    for i in range(0,TRAIN_SIZE,TRAINING_BATCH_SIZE):
                        ...
                        x2_test    = torch.unsqueeze(uncertainty_states[i], 0)  #states[i][1]#Uncertainty
                        ...
                        test_rmse_steer_Y, test_rmse_steer_Yhat = [],[]
                        test_rmse_throttle_Y, test_rmse_throttle_Yhat = [],[]
                        test_rmse_brake_Y, test_rmse_brake_Yhat = [],[]
                        ...
                        for k in range(len(y_test)):
                            ...
                            test_rmse_steer_Y.append(y_test[k,0].item())
                            test_rmse_throttle_Y.append(y_test[k,1].item())
                            test_rmse_brake_Y.append(y_test[k,2].item())

                            test_rmse_steer_Yhat.append(yhat_test[k,0].item())
                            test_rmse_throttle_Yhat.append(yhat_test[k,1].item())
                            test_rmse_brake_Yhat.append(yhat_test[k,2].item())
                        ...
                        test_rmse_steer_Yhat = torch.Tensor(test_rmse_steer_Yhat)
                        test_rmse_throttle_Yhat = torch.Tensor(test_rmse_throttle_Yhat)
                        test_rmse_brake_Yhat = torch.Tensor(test_rmse_brake_Yhat)

                        test_rmse_steer_Y = torch.Tensor(test_rmse_steer_Y)
                        test_rmse_throttle_Y = torch.Tensor(test_rmse_throttle_Y)
                        test_rmse_brake_Y = torch.Tensor(test_rmse_brake_Y)

                        y_test = y_test.cpu().numpy()
                        yhat_test = yhat_test.cpu().numpy()

                        tb.add_scalar("validation_accuracy_steer", test_accuracy_steer, epoch_itter)
                        tb.add_scalar("validation_accuracy_throttle", test_accuracy_throttle, epoch_itter)
                        tb.add_scalar("validation_accuracy_brake", test_accuracy_brake, epoch_itter)
                        tb.add_scalar("validation_accuracy_avg", test_accuracy_avg, epoch_itter)
                        
                        tb.add_scalar("Steer Validation RMSE", math.sqrt(mean_squared_error(test_rmse_steer_Y, test_rmse_steer_Yhat)), epoch_itter)
                        tb.add_scalar("Throttle Validation RMSE", math.sqrt(mean_squared_error(test_rmse_throttle_Y, test_rmse_throttle_Yhat)), epoch_itter)
                        tb.add_scalar("Brake Validation RMSE", math.sqrt(mean_squared_error(test_rmse_brake_Y, test_rmse_brake_Yhat)), epoch_itter)
            ...
            tb.close()
            torch.save(self.model,f'imitation_models/55kGT_5Layers.pt')  
            torch.cuda.empty_cache()
